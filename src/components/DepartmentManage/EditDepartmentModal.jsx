...
function EditDepartmentModal({ deptData, onClose, onEdit }) {
    ...
    const handleChange = (e) => {
        ...
        setFormData((prev) => ({
            ...prev,
            [name]: name === "departmentName" ? value : value,
        }));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${apiUrl}/api/dept/updateDepartment`, submitData);
            if (response.status === 200) {
                alert(response.data.message || "Department updated successfully.");
                onEdit?.(response.data.department);
                onClose();
            }
        } catch (error) {
...
const Input = ({ label, name, type = "text", value, onChange, error, readOnly, ...props }) => (
    <div>
        ...
        <input
            ...
            className={`w-full p-2.5 border rounded-lg 
                ...
                text-gray-900 focus:ring-2 outline-none transition
