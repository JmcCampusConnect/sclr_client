...
function StudentManageTable({ students: initialStudents, onSaveStudent }) {
    ...
    const handleMobileChange = (e, registerNo) => {
        const newMobile = e.target.value;
        setStudents(prevStudents =>
            prevStudents.map(student =>
                student.registerNo === registerNo
                    ? { ...student, mobileNo: newMobile, isEdited: true }
                    : student
            )
        );
    };

    const handleAadharChange = (e, registerNo) => {
        const newAadhar = e.target.value;
        setStudents(prevStudents =>
            prevStudents.map(student =>
                student.registerNo === registerNo
                    ? { ...student, aadharNo: newAadhar, isEdited: true }
                    : student
            )
        );
    };
            ...
            <div className="max-h-[700px] overflow-y-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-center table-auto">
                    <thead className="bg-gray-100 dark:bg-gray-900 sticky top-0 z-10">
                        <tr>
                            {["S.No", "Register No", "Name", "Department", "Category", "Mobile No", "Aadhar No", "Password", "Sembased", "Action"].map(
                                (header) => (
                                    <th
                                        key={header}
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {students.length > 0 ? (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-4">
                                        <input
                                            ...
                                            onChange={(e) => handleMobileChange(e, student.registerNo)}
                                    ...
                                    <td className="px-4 py-4">
                                        <input
                                            ...
                                            onChange={(e) => handleAadharChange(e, student.registerNo)}
                                            className="w-28 sm:w-40 px-3 py-1.5 border border-gray-300 rounded-lg text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition dark:bg-gray-900 dark:text-gray-100 dark:border-gray-600"
                            ...
                            <tr>
                                <td colSpan="11" className="p-4 text-center text-gray-500 dark:text-gray-400 text-sm sm:text-base">
                                    No students found.
                                </td>
                            </tr>
                        )}
